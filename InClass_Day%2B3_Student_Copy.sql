...
# Bank_inventory_pricing  during 2nd month . 
 select Product,count(Product),avg(estimated_sale_price)
...
select Product,max(purchase_cost)-min(purchase_cost)  from Bank_inventory_pricing
group by Product;
# Question 10:
# 10) Print the sum of Purchase_cost of Bank_inventory_pricing during 1st and 2nd month
select sum(purchase_cost)  from Bank_inventory_pricing group by month 
having month in (1,2);
# Question 11:
...
select Product,sum(purchase_cost),avg(purchase_cost)
from Bank_inventory_pricing 
group by Product
...
select max(estimated_sale_price),sum(Quantity)
from Bank_inventory_pricing 
...
select product ,avg(estimated_sale_price),avg(purchase_cost),avg(estimated_sale_price)*1.15 as newcost
from Bank_inventory_pricing 
group by product
